...
for (const [key,value] of Object.entries(data)){
    ...
    if (typeof value === typeof {}){
        ...
        textArea.innerHTML += `<div style="color:green">${key}</div> =  </br>`
        for (const [subKey,subValue] of Object.entries(value)){
            ...
            if (typeof subValue === typeof{}){
                for (const [subsubKey,subsubValue] of Object.entries(subValue)){
                    ...
                    const subKeyColor=`${subsubKey}`
                    textArea.innerHTML += `<div class="subsubValue">${subsubKey} = ${subsubValue} </br></div>`
                }
            }
        }
    }
else {
    textArea.innerHTML += `${key} = ${value} </br>`
}
}
